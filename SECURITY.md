...
If you discover a security issue in cloud-itonami-isic-1073, **do not open a
...
This repository implements the **Governor** for cocoa/chocolate/sugar-
confectionery plant operations — a deterministic safety-checking layer that
...
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:cadmium-exceeds-max`,
   `:foreign-material-detected`, `:allergen-label-mismatch`) blocks the
